# file to train attentional model on cub dataset
# this specific file tries to use half precision just to save data, since we have a lot of parameters
...
from Losses.Loss import attentional_adv_D, adv_G, calculateAttentionMatchingScoreBatchWrapper, totalLoss
from Modules.ChannelAttentionalDiscriminator import ChannelAttentionalDiscriminator
from Modules.ChannelAttentionalGenerator import ChannelAttentionalGenerator
...
def train(dataloader, generator, discriminator, textEncoder, imageEncoder, device, optimizerG, optimizerD,
    ...
    # training
    trainFile = os.path.join(os.getcwd(), 'attentionalCubTrainingCheckpoint.pth')
    # summary writer for keeping track of training
    writer = SummaryWriter('AttentionalDAMSMWriterStuff')
    for epoch in tqdm(range(epochNum+1,maxEpoch+1)):
        ...
        for step in tqdm(range(len(dataIterator))):
            ...
            with torch.cuda.amp.autocast():
                # getting pad mask
                padMask = torch.arange(0, 18, step=1).long().expand(batch_size, 18).to(device).detach()

                # Forward pass for generator to get generated imgs conditioned on embedded text
                x_fake = generator(z, sentEmbeddings,wordEmbeddings, padMask)
            ...
            # new word embedding stuff to require grad for the word embeddings fed into discriminator
            wordEmbeddings = (wordEmbeddings.data).requires_grad_()

            # 0 optimizer gradients before computing loss
            optimizerD.zero_grad()
            # Compute advesarial loss for discriminator
            L_advD, _ = attentional_adv_D(D=discriminator,
                                   opt=optimizerD,
                                   x=trainImages,
                                   ...
                                   padMask = padMask,
                                   device=device,
                                   lambda_MA=2, p=6, scaler = scaler)

            scaler.scale(L_advD).backward(retain_graph=True)
            scaler.step(optimizerD)


            # update G
            with torch.cuda.amp.autocast():
                D_fake = discriminator(x_fake,sentEmbeddings,wordEmbeddings,padMask)
...
if __name__ == '__main__':
    ...
    generator = ChannelAttentionalGenerator(z_shape=z_shape).to(device)
    discriminator = ChannelAttentionalDiscriminator(in_dim).to(device)
